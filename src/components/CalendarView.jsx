...
// ── EINZELNES EVENT IM TAGES-MODAL ────────────────────────────────────────────
function EventItem({ e }) {
  ...
  return (
    <div style={{
      ...
      border: `1px solid ${ic}55`, overflow: "hidden",
    }}>
      {/* Header */}
      <div
      ...
      >
        ...
        <div style={{ flex: 1 }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap", marginBottom: 8 }}>
            <span style={{
              fontSize: 13, fontWeight: 700, color: ic,
        ...
        {/* Expand Arrow */}
        <span style={{
          color: C.textLow, fontSize: 22, flexShrink: 0, marginTop: 4,
    ...
    <Modal onClose={onClose} maxWidth={900} accentColor={accentCol}>
      <div style={{ padding: "40px 48px 40px" }}>
        ...
        <div style={{ marginBottom: 32 }}>
          <div style={{
            fontSize: 14, color: C.textLow,
            letterSpacing: "0.08em", marginBottom: 8,
            textTransform: "uppercase", fontWeight: 700,
          }}>{dayLabel}</div>
          <h2 style={{
            fontFamily: FONT.serif, fontSize: 34, color: C.textHi,
        ...
        {events.length > 0 ? (
          ...
          <div style={{
            ...
            <div style={{ fontFamily: FONT.serif, fontSize: 20, color: C.textMid, marginBottom: 8 }}>
        ...
        {events.length > 0 && (
          <div style={{
            ...
            <span>💡</span>
            Klick auf ein Event zum Auf-/Zuklappen der vollständigen Analyse mit BTC/ETH-Impact
          </div>
        )}
      </div>
    </Modal>
    ...
    <>
      <div
        ...
        style={{
          ...
          opacity: isEmpty ? 0.3 : 1, cursor: "pointer",
      ...
      >
        ...
        {events.map((ev, i) => (
          <div key={i} style={{
            background: IMP_COL[ev.imp] || C.gray,
        ...
        {!isEmpty && hovered && (
          <div style={{ fontSize: 9, color: C.blue, marginTop: 4, fontWeight: 600 }}>→ Tag öffnen</div>
        )}
      </div>
...
export default function CalendarView() {
  return (
    <div>
      {/* Legende */}
      <div style={{
        display: "flex", gap: 20, fontSize: 13, color: C.textMid,
        marginBottom: 22, flexWrap: "wrap", alignItems: "center",
      }}>
        {[["H","Hoher Impact",C.impH],["M","Mittlerer Impact",C.impM],["L","Niedriger Impact",C.impL],["N","Feiertag",C.impN]].map(([k,l,c])=>(
          <div key={k} style={{ display: "flex", alignItems: "center", gap: 7 }}>
            <span style={{ width: 10, height: 10, borderRadius: 3, background: c, display: "inline-block" }} />
            <span>{l}</span>
          </div>
        ))}
        <span style={{ color: C.textLow, marginLeft: "auto", fontSize: 12 }}>
          Klick auf Tag → großes Tages-Modal mit vollem Detail
        </span>
      </div>
      ...
      {CALENDAR_WEEKS.map((week, wi) => (
        <div key={wi} style={{ marginBottom: 24 }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(7,1fr)", gap: 5, marginBottom: 6 }}>
            {WEEK_DAYS.map(d => (
              <div key={d} style={{ fontSize: 11, fontWeight: 700, color: C.textMid, textAlign: "center", padding: "4px 0" }}>{d}</div>
            ))}
          </div>
